...
async function beginIndex(gidNodes, service) {
  await Promise.all(gidNodes.get(service).map((node) => {
...
async function setup(gidCounts, job) {
  ...
  await Promise.all([...gidNodes.entries()].map(([gid, nodes]) => createGroup({gid}, nodes)));
  ...
  await Promise.all(nodes.map((node) => local.comm.send([], {...stop, node})));
  await new Promise((res) => server.close(res));
  return result;
}

test('stuff', () => setup({client: 1, students: 2, courses: 3}, async (gidNodes) => {
  ...
  let remote;
  remote = {service: 'students', method: 'listTokens', node: gidNodes.students[0]};
  const r0 = await local.comm.send([], remote);
  remote = {service: 'students', method: 'listTokens', node: gidNodes.students[1]};
  ...
  expect(async () => {
    ...
    remote = {service: 'students', method: 'lock', node: gidNodes.students[1]};
    await local.comm.send(['CSCI 1380', r0[0]], remote);
  }).rejects.toThrow();

  remote = {service: 'students', method: 'lock', node: gidNodes.students[0]};
  ...
  remote = {service: 'students', method: 'unlock', node: gidNodes.students[0]};
  await local.comm.send(['CSCI 1380', lock1380, r0[0]], remote);

  // submit on 1270
  remote = {service: 'students', method: 'submit', node: gidNodes.students[0]};
  await local.comm.send(['CSCI 1270', lock1270, r0[0]], remote);

  let taking;
  remote = {service: 'students', method: 'listRegister', node: gidNodes.students[0]};
  ...
  remote = {service: 'client', method: 'register', node: gidNodes.client[0]};
  await local.comm.send(['AFRI 0001', r0[0]], remote);

  remote = {service: 'students', method: 'listRegister', node: gidNodes.students[0]};
  taking = await local.comm.send([r0[0]], remote);
  expect(taking).toEqual(expect.arrayContaining(['CSCI 1270', 'AFRI 0001']));

  // TODO: add test students to synthesize
  // TODO: test course with prerequisites
  remote = {service: 'client', method: 'coursesTaking', node: gidNodes.client[0]};
  taking = await local.comm.send(['AFRI 0001'], remote);
  expect(taking).toEqual(expect.arrayContaining([r0[0]]));

  remote = {service: 'client', method: 'studentsTaking', node: gidNodes.client[0]};
  taking = await local.comm.send([r0[0]], remote);
